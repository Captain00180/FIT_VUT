...
void htDelete ( tHTable* ptrht, tKey key ) {
    ...
    {// search for predecessor
        while (item != NULL && item->ptrnext != NULL && strcmp(key, item->ptrnext->key))
        {
            item = item->ptrnext;
        }
        if (item == NULL || item->ptrnext == NULL)
            return;
        //item.next == item to be deleted
        tHTItem *delete = item->ptrnext;
        item->ptrnext = delete->ptrnext;
