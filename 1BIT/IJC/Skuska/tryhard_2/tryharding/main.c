...
int f3(void) {
    static char str[10] = "ABC1234D";   // Statické pole 10 znaků. Víckrát se funkce nevolá takže je mi jedno že to je static
...
char f7(void) {
    char *s[] = { "1234", "5678", "BCD", };  // Pole ukazatelů na chary. Stringy jsou v .data readonly
...
char f9(void) {
    const char *s[] = { "0123", "4567", "9AB", };  // Pole ukazatelů na chary. Stringy jsou v .data readonly
...
{
    double (*ptr)[3] = malloc(9 * sizeof(double));
    if(double != NULL)
    {
        for(int i = 0; i < 3; ++i)
        {
...
{
    int (*ptr)[100][100] = calloc(10 000, sizeof(int))
}
...
inline void *func(size_t N)
{
    int (*ptr)[N][N] = calloc(N, sizeof(int));
    if(ptr != NULL)
    {
        for(int i = 0; i < N; ++i)
        {
...
#define ASSIGN_BITFIELD(x, pos, n, val)\
    assert(pos >= 0 && n > 0);\
    assert (pos < sizeof(unsigned long long) * CHAR_BIT && n < sizeof(xxx);\
    x = (~(unsigned long long)(~0) << pos) & x) | (~(unsigned long long)(~0) << pos + n) & x) | (   (unsigned long long)(0) | val) << pos
...
#define
    ...
    x = ((unsigned) x << n) | (unsigned)x >> (sizeof(x) * 8)
...
// 3) Definujte makro \n_BITS(x, pos, n), které v L-hodnotě x typu
...
#define DELETE_N_BITS(x, pos, n)\
    assert(n > 0 && pos >= 0);\
    assert(n < sizeof(x) * CHAR_BIT && pos < sizeof(x) * CHAR_BIT);\
    if( ((unsigned)x) >> (sizeof(x) * 8 - 1) )\
        x = (~((unsigned)(~0) << pos) & x) | ((~((unsigned)(~0) << (pos + n))) & (unsigned)x) >> n | ((unsigned)(~0) << (pos + n))


    assert(pos < sizeof(signed int) * CHAR_BIT && n < sizeof(signed int) * CHAR_BIT);\
    assert(pos >= 0 && n > 0);\
    if(  (unsigned int) 1 << (sizeof(int) * CHAR_BIT - 1) & x )\
        x = (~(unsigned) (~0) << pos) & x | ((unsigned)(~0) << pos + n) & x) >> n) | ((unsigned)(~0) << pos+n)
...
bool ulong2str(unsigned long x, int r, char *s, int n)
{
        ...
        int mod = 0;
        while(counter < (n-1) && x != 0)
        {
            mod = x % r;
            x = x / r;
            temp[counter] = num_to_char(mod);
            ++counter;
        }
        if(x != 0)
            return false;
...
const char *StrStr(const char *Str, const char *s1)
{
    if(Str == NULL || s1 == NULL || strlen(Str) == 0 || strlen(s1) == 0)
        return NULL;
...
int SearcReplace(char *Str, int Max, const char *s1,const char *s2)
{
...
bool Edit(char*Str, int Max, unsigned Pos, unsigned N, const char *Ins)
{
        if(Str == NULL || Ins == NULL || Pos <= 0 )
            return false;
        if(strlen(Str) == 0 || strlen(Str) + strlen(Ins) - N >= Max || Pos >= strlen(Str) )
            return false;
        if(N == 0)
            return true;

        int diff = N - strlen(Ins);
        // shift left
        if(diff > 0)
        {
...
int ReadCSVItem (FILE *f, char Delimiter, char *Str, unsigned m)
{
        if (f == NULL || Str == NULL || m <= 0)
            return -42;
        char c = fgetc(f);

        int counter = 0;
        while(c != EOL && c != EOF && c != Delimiter)
        {
            ++counter;
            if(counter < m)
                Str[counter - 1] = c;
...
int main(int argc, char *argv[])
{
        ...
        while(c != EOF) {
            char *arr[1024] = 0;
            int counter = 0;
            while(c != EOF && c != -42 && counter < 1024)
            {
                c = ReadCSVItem(file, ';', arr[counter], 128);

                ++counter;
                ++lines;
            }
            char *temp = arr[argv[1]];
            arr[argv[1]] = arr[argv[2]];
            arr[argv[2]] = temp;
            for(int i = 0; i < counter; ++i)
            {
                printf("%s", arr[i]);
...
int ReplaceSP(char *Str, int Max, const char *s2)
{
        if(Str == NULL || s2 == NULL || Max <= 0)
            return 0;
        int x = 0;
        char * ptr = (char*)SearchSP(Str, &x);
        if (ptr == NULL)
            return 0;

        // HelloXxXWorld0d0d0...
        // XxX
        int diff = x - strlen(s2);
        if(diff > 0)
        {
...
int main(int argc, char *argv[])
{
        ...
        while(c != EOF)
        {
            c = CopyNonUPPER(file, stdout);
            if (c == EOF)
                return 0;
            char str[128] = 0;
            c = ReadUPPER(str, 128, file);
            strrev(str);
            prinf("%s\n", str);
            if(c == EOF)
                return 0;
...
const char *WordSearc(const char *Str, int *ptr)
{
        if(Str == NULL)
            return NULL;
        if(strlen(Str) == 0)
        ...
        for(int i = 0; i < strlen(Str); ++i)
        {
            ...
            if(!isalpha(Str[i]) && chr)
                if(ptr != NULL)
                    ptr = counter;
                return (Str + i - counter)
        }
        return NULL;
}
// ???xxx???0..
...
int WordReplace(char *Str, int Max, const char *s2)
{
        ...
        int x = 0;
        char *temp = (char*) WordSearc(Str, x);
        if(temp == NULL)
            return 0;
        int diff = x - strlen(s2);
        if(diff > 0)
