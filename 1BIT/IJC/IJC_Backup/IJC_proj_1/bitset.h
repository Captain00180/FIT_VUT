...
inline int bitset_getbit(bitset_t jmeno_pole, bitset_index_t index)
	{
		if (index >= bitset_size(jmeno_pole))
		{
			error_exit("bitset_getbit: Index %lu mimo rozsahu 0..%lu",(bitset_index_t) index,(bitset_index_t) bitset_size(jmeno_pole)-1);
		}
		return jmeno_pole[1 + index/UL_SIZE] & 1L << (index % UL_SIZE);
	}
#else
